...
const EditResourcesModal = ({ isOpen, onOpenChange, recurseId }) => {
  ...
  useEffect(() => {
    fetch(
      `https://backend.driveshikhun.com/api/v1/recurse/getRecursesById/${recurseId}`
  ...
  const handleUpdate = async (onClose) => {
    const url = `https://backend.driveshikhun.com/api/v1/recurse/updateRecurses/${recurseId}`;
    try {
      const response = await axios.patch(url, {...formData,courseName:selectedCourse?.title,recurseFile:img?img:formData?.recurseFile});
      toast.success("data updated");
      onClose();
    } catch (error) {
      console.error("recurse updating live class:", error);
    }
  };
  return (
    <>
