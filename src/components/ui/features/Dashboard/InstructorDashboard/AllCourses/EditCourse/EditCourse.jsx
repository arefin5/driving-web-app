...
const EditCourse = ({ isOpen, onOpenChange, courseId }) => {
  ...
  useEffect(() => {
    fetch(`https://backend.driveshikhun.com/api/v1/course/getCoursesById/${courseId}`)
      .then((res) => res.json())
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!formData?.title && !formData?.category) {
      ...
      const data = {
        ...formData,
        courseThumbnail: img?img:formData?.courseThumbnail,
        userId: id,
        introVideo:introVideo?introVideo:formData?.introVideo,
      };
      try {
        UpdateHooks(
          `https://backend.driveshikhun.com/api/v1/course/updateCourses/${courseId}`,
          data
        );
      } catch (error) {
                ...
                <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="bg-[#F2F2F2] rounded-lg mt-2 py-3 px-4 border border-solid border-black/30 ">
                      ...
                      <div className="grid grid-cols-2 gap-x-5 gap-y-2">
                        {[...Array(8)].map((_, index) => (
                          <input
                            key={index}
                            className="bg-white border border-solid border-black/30 placeholder:text-black/50 rounded py-[6px] px-4 placeholder:text-xs outline-0  dark:placeholder:text-white dark:bg-transparent dark:border-white dark:text-white"
                            type="text"
                            value={formData?.itemsWithinCourse[index]}
                  ...
                  <div>
                    ...
                    <div className="bg-[#F2F2F2] mt-2 rounded-lg py-3 px-4 border border-solid border-black/30 ">
                      <p className="mb-3 text-xs text-black/50 dark:text-white font-light">
                        Add minimum 4 items
                      </p>
                      <div className="grid grid-cols-2 gap-x-5 gap-y-2">
                        {formData?.courseFacilities?.map((_, index) => (
                          <input
                            key={index}
                            className="bg-white border border-solid border-black/30 placeholder:text-black/50 rounded py-[6px] px-4 placeholder:text-xs outline-0  dark:placeholder:text-white dark:bg-transparent dark:border-white dark:text-white"
                            type="text"
                            value={formData?.courseFacilities[index]}
