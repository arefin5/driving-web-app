...
const AddModuleModal = ({ isOpen, onOpenChange,setRefetch, refetch }) => {
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    const data = { ...formData, userId: id,courseId:addCourseLesson,date,textInstruction:content };
    console.log(data)
    try {
      await PostHooks(
        `https://backend.driveshikhun.com/api/v1/module/addModules`,
        data,
        "Module Posted!",refetch,setRefetch
      );
      onOpenChange();
    } catch (err) {
      console.log(err);
