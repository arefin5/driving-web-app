...
const EditRecordLiveClassModal = ({
  isOpen,
  onOpenChange,
  selectedLiveClassId,
}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://backend.driveshikhun.com/api/v1/recordedClasses/getRecordedClassesById/${selectedLiveClassId}`
        );
    ...
    fetchData();
  }, [selectedLiveClassId, isOpen]);

  useEffect(() => {
    fetch(
      `https://backend.driveshikhun.com/api/v1/course/specific?fieldName=userId&&fieldValue=${id}`
    ).then((res) => res.json().then((data) => setCourse(data?.data)));
  }, [id, isOpen]);
  ...
  const handleUpdate = async (e) => {
    e?.preventDefault();
    const url = `https://backend.driveshikhun.com/api/v1/recordedClasses/updateRecordedClasses/${selectedLiveClassId}`;
  ...
  console.log("live class check", formData.startingDate);
        ...
        <ModalContent className="relative modal_scrollbar">
          <>
            ...
            <ModalHeader className="flex items-center gap-1">
              ...
              <div>
                <h2 className="text-[32px] leading-[3rem] font-semibold text-transparent bg-loginButton bg-clip-text dark:text-white">
                  রেকর্ডেড লাইভ ক্লাস ডিটেইলস আপডেট করুন
                </h2>
            ...
            <ModalFooter>
              ...
              >
                Save Recorded Live Class Details
...
export default EditRecordLiveClassModal;
