...
const AddRecordLiveClassModal = ({ isOpen, onOpenChange }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://backend.driveshikhun.com/api/v1/recordedClasses/addRecordedClasses",
        { ...formData, userId: id }
      );

      if (response.status === 200) {
        console.log("Recorded Live Class added successfully!");
        handleConfrimModal(onOpenChange);
      } else {
        console.error("Failed to add Recorded Live Class. Server responded with:", response);
      }
    } catch (error) {
      console.error("Error adding Recorded Live Class:", error.message);
    }
  };
...
export default AddRecordLiveClassModal;
