...
const ViewQuizDetailsModal = ({ isOpen, onOpenChange }) => {
  const {
    onOpen: onOpenEdit,
    isOpen: isOpenEdit,
    onOpenChange: onOpenChangeEdit,
  } = useDisclosure();
  const handleViewQuiz = (onClose) => {
    onOpenEdit();
    onClose();
  };
  return (
    <>
        ...
        <ModalContent className="relative modal_scrollbar">
          {(onClose) => (
            <>
              ...
              <ModalFooter>
                <button
                  onClick={() => handleViewQuiz(onClose)}
                  type="submit"
      ...
      {/* Handle open modal */}
      <AddQuizModal isOpen={isOpenEdit} onOpenChange={onOpenChangeEdit} />
    </>
  );
};
