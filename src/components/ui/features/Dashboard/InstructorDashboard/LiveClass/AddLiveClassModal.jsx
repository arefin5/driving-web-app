...
const AddLiveClassModal = ({ isOpen, onOpenChange }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "selectCourse") {
      const selectedCourse = course.find((course) => course.title === value);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await postData(
        "https://backend.driveshikhun.com/api/v1/liveClasses/addLiveClasses",
        { ...formData, courseId: selectedCourseId, userId: id }
      );
      ...
      if (response.ok) {
        ...
        console.error(
          "Failed to add Live Class. Server responded with:",
          response
        );
  ...
  return (
    <>
      ...
      {/* Handle open modal */}
      <ConformationModal
        message="add live class added successfully"
        isOpen={isOpenConfrimModal}
        onOpenChange={onOpenChangeConfrimModal}
      />
    </>
