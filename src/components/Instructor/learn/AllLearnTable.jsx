...
import UpdateLearnModal from "./UpdateLearnModal";
...
const AllLearnTable = () => {
  ...
  const [learn] = useGetDataHook(
    `https://backend.driveshikhun.com/api/v1/learn/specific?fieldName=userId&&fieldValue=${user?._id}`,refetch
  );
          ...
          <tbody>
            {learn?.map((l) => (
              ...
              >
                ...
                >
                  <span className=""> {convert(l?.learnDescription?.slice(0,300))}</span>
                </td>
