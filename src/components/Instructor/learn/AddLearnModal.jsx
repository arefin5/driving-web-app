...
const AddLearnModal = ({ isOpen, onOpenChange }) => {
  ...
  const handelSubmit = async (onClose) => {
    const data = {
      learnVideo: video,
      learnImg: img,
      learnDescription: content,
      userId: user?._id,
      courseId: selectedCourse,
      moduleId: selectedModule,
      lessonId: selectedLesson,
    };
    if (selectedCourse && selectedLesson && selectedModule) {
      try {
        await PostHooks("https://backend.driveshikhun.com/api/v1/learn/addLearns", data);
      } catch (error) {
        toast.error(error);
      }
      onClose();
