...
const AllPendingCourseTable = () => {
  ...
  const { data: pendingCourse, refetch } = useAllStatusCourse(
    "pending",
    currentPage,
    itemsPerPage
  );
  const { refetch: confirmRefetch } = useAllStatusCourse(
    "confirm",
    currentPage,
    itemsPerPage
  );
  const totalData = pendingCourse?.total;
...
export default AllPendingCourseTable;
