...
const ApprovedInstructorList = () => {
  ...
  const { data: approvedInstructor, refetch } = useInstructorData(
    "instructor",
    "success",
    currentPage,
    itemsPerPage
  ...
  const totalData = approvedInstructor?.total;
    ...
    <>
      <div className="relative border border-solid border-black/30 rounded-xl h-[600px] !max-h-full overflow-y-auto overflow-x-scroll lg:overflow-x-hidden lg:scrollbar-hide">
        <table className="w-full max-lg:min-w-[800px]  px-10 table-auto text-sm text-left overflow-hidden  rounded-xl">
          ...
          {approvedInstructor?.data?.length === 0 ? (
            <NoUserFound message={"No user found"} />
          ) : (
            <tbody>
              {approvedInstructor?.data?.map((instructor) => (
                <tr
                  key={instructor._id}
                ...
                >
                  ...
                  <td className="pr-4 py-3 space-x-3 text-center border-collapse">
      ...
      <ConformationModal
        isOpen={isopenConfrom}
        onOpenChange={onOpenChangeConfrom}
        message="Instructor Moved Pending successfully"
      />
    </>
  );
};

export default ApprovedInstructorList;
