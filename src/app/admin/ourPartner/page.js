...
const OurPartner = () => {
  ...
  //   load data
  useEffect(() => {
    fetch(`https://backend.driveshikhun.com/api/v1/partners/getPartners`)
      .then((res) => res.json())
      .then((data) => {
        if (data?.data.length) {
...
const handleSubmit = async (e) => {
  ...
  try {
    if (formData?._id) {
      await UpdateHooks(
        `https://backend.driveshikhun.com/api/v1/partners/updatePartners/${formData?._id}`,
        { ...formData, img: imageUrl }
      );
    } else {
      const response = await fetch("https://backend.driveshikhun.com/api/v1/partners/addPartners", {
        method: "POST",
        headers: {
                    ...
                    <button
                      ...
                      onClick={() => {
                        DeleteHook({
                          setRefetch,
                          refetch,
                          url: `https://backend.driveshikhun.com/api/v1/partners/deletePartners/${partner?._id}`,
                        });
                      }}
                    >
                      <Icon
